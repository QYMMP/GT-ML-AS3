...
class DataLoader(ABC):
    ...
    def load_and_process(self, data=None, preprocess=True):
        ...
        if data is not None:
            ...
            self._load_data()
        self.log("Processing {} Path: {}, Dimensions: {}",
                 self.data_name(), self._path, self._data.shape)
        if self._verbose:
        ...
        self.log("Class distribution (%): {}",
                 (class_dist / self.classes.shape[0]) * 100)
    ...
    def dump_test_train_val(self, test_size=0.2, random_state=123):
        ...
        tst.to_csv('data/{}_test.csv'.format(self.data_name()),
                   index=False, header=False)
        trg.to_csv('data/{}_train.csv'.format(self.data_name()),
                   index=False, header=False)
        val.to_csv('data/{}_validate.csv'.format(self.data_name()),
                   index=False, header=False)
...
class CreditDefaultData(DataLoader):
    ...
    def __init__(self, path='datasets/default of credit card clients.xls', verbose=False, seed=1):
...
class TicTacToeData(DataLoader):

    def __init__(self, path='datasets/tic-tac-toe.data', verbose=False, seed=1):
        super().__init__(path, verbose, seed)
    ...
    def _preprocess_data(self):
        ...
        oe = preprocessing.OrdinalEncoder().fit(X)
        print(oe.categories_)
        X = oe.transform(X)
        y = preprocessing.LabelEncoder().fit_transform(y)
        finY = np.atleast_2d(y).T
        self._data = pd.DataFrame(np.hstack((X, finY)))
...
class PenDigitData(DataLoader):
    def __init__(self, path='datasets/pendigits.csv', verbose=False, seed=1):
        super().__init__(path, verbose, seed)
...
if __name__ == '__main__':
    ...
    ca_data = TicTacToeData(verbose=True)
